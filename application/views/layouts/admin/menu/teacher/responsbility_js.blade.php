...
$(document).ready(function() {
    let url = '{{ url('/') }}/teacher/responsbility';
    ...
    $('#detail_yearly_period').select2({
        dropdownParent:$("#modal_detail"), //If Select2 Inside Modal, $(".jconfirm-box-container") if jConfirm
    ...
    //Datatable
    let data_table = $("#table_detail").DataTable({
        "responsive": true,
        "serverSide": true,            
        "ajax": {
            url: url,
        ...
        "columnDefs": [
            ...
            {"targets":1, "width":"20%", "title":"Grade", "searchable":true, "orderable":true},      
            {"targets":2, "width":"20%", "title":"Department", "searchable":true, "orderable":true},                        
                ...
                render: function(data, meta, row) {
                    ...
                    <div class="dropdown d-inline-block">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                                <a href="#" class="dropdown-item btn_edit" data-id="${data}" data-session="${row.detail_session}" data-name="${row.lesson_name}">
                                    <i class="ri-eye-fill align-bottom me-2 text-muted"></i> View
                                </a>
                            </li>
                            <li>
                                <a href="#" class="dropdown-item remove-item-btn btn_delete" data-id="${data}" data-session="${row.detail_session}" data-name="${row.lesson_name}" style="cursor:pointer">
    ...
    $("#table_detail_filter").css('display','none');
    $("#table_detail_length").css('display','none');
    $("#filter_length").on('change', function(e){
        var value = $(this).find(':selected').val(); 
        $('select[name="table_detail_length"]').val(value).trigger('change');
    ...
    $(document).on("click","#btn_save", function(e) { // Done
        ...
        if(next){
            if ($("#detail_lesson").find(":selected").val() == 0) {
                next = false;
                notif(0,'Lesson required');
            }
        }    
        ...
        if(next){
            let form = new FormData($("#form_detail")[0]);
            ...
            if(detailSession == ''){
                // var set_url = url_api + 'user-create';
            }else{              
                form.append('detail_session',detailSession);
                // var set_url = url_api + 'user-update';
            }
            $.ajax({
                type: "post",
                // url: set_url,
            ...
            $.ajax({
                ...
                success:function(d){
                    ...
                    if(parseInt(s) == 1){
                        ...
                        //     $("select[id='grade']").append(''+'<option value="'+r.grade_session+'">'+r.grade_name+'</option>');
                        //     $("#grade").val(r.grade_session).trigger('change');
                        // }  
                        // if((r.department_session) && r.department_session.length > 10){
                        //     $("select[id='grade_department']").append(''+'<option value="'+r.department_session+'">'+r.department_name+'</option>');
                        //     $("#grade_department").val(r.department_session).trigger('change');
                        // }    

                        if((r.lesson_session) && r.lesson_session.length > 10){
                            $("select[id='detail_lesson']").append(''+'<option value="'+r.detail_lesson+'">'+r.lesson_name+' ['+r.grade_name+' '+r.department_name+']</option>');
                            $("#detail_lesson").val(r.detail_lesson).trigger('change');
                        }                                                                        
                    }else{
                        notif(s,m);
                        detailSession = '';
    ...
    function formDetailReset(){
        detailSession = '';
        $("#form_detail input")
        .not("input[name='detail_status']").val('');
        $("input[name=detail_status][value=1]").prop("checked", true).change();
        $("#form_detail select").val(0).trigger('change');    
